...
function derefSchema(schema, options, missing, fn) {
  traverse(schema, function (node, next) {
      ...
      getRefSchema(schema, node, options, function (err, newValue) {
        if (newValue) {
          derefSchema(newValue, options, missing, function (err, value) {
            if (self.parent && (value || newValue)) {
              self.parent[self.key] = value || newValue;
...
function deref(schema, options, fn) {
  ...
  function keepGoing(kgSchema, missing) {
    var refs = propSearch.searchForExistence(kgSchema, '$ref');

    if (refs && refs.length === 0) {
      return false;
    }

    var refValues = _.chain(refs)
      .map(utils.getRefValue)
      .reject(_.isEmpty)
      .uniq()
      .value();
    ...
    var diff = _.difference(refValues, missing);
  ...
  async.whilst(function () {
      return keepGoing(baseSchema, missing);
    },
    function (cb) {
      return derefSchema(baseSchema, options, missing, cb);
    },
    function (err, resSchema) {
      return fn(err, resSchema || baseSchema);
    });
}
