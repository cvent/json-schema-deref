...
function getRefSchema(refVal, refType, parent, options, state, fn) {
  ...
  if (refType === 'file') {
    ...
    fullRefFilePath = path.isAbsolute(filePath) ? filePath : path.resolve(state.cwd, filePath);
  }
...
/**
 * Determines if there are any $refs left in schema that we need to deref, taking account the missing
 * $refs we've already accumulated.
 * @param schema
 * @param missing
 * @param type
 * @returns {*|boolean}
...
/**
 * Deref schema. web, file and custom $ref types are done first, then local
 * @param schema
 * @param options
 * @param state
 * @param fn
